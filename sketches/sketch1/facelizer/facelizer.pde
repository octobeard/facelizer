...
// AUDIO DATA
String          audioFile    = "shelera.aiff";
String          audioPath    = PATH_DATA + audioFile;
AudioProcessor  audio;

// RENDERING
String      renderFileName   = "el-01/el-";
...
void setup() {
    ...
    img1 = loadImage(PATH_DATA + "el.png");
    img2 = loadImage(PATH_DATA + "el-mask.png");
...
void draw() {   
    ...
    for (HDrawable d : pool) {
        if(d.bool("active")) {
            ...
            float depth = map(fftData, 0, 100, 8, 30);
            float z = map(fftData, 0, 100, -100, 100);
            s.z(z);
            s.depth((int) depth);
            if (count == 10) {
                //println(depth);
